...
copy_files() {
    ...
    if [[ "${DOWNLOAD_FROM_GITHUB}" == "true" ]]; then
        ...
        for file in app.py mpv_controller.py requirements.txt fix_blank_screen.sh blank_screen_now.sh blank-screen.service; do
    ...
    # Make Python files and scripts executable
    ...
    if [[ -f "${INSTALL_DIR}/fix_blank_screen.sh" ]]; then
        chmod +x "${INSTALL_DIR}/fix_blank_screen.sh" || error_exit "Failed to make fix_blank_screen.sh executable"
    fi
    
    if [[ -f "${INSTALL_DIR}/blank_screen_now.sh" ]]; then
        chmod +x "${INSTALL_DIR}/blank_screen_now.sh" || error_exit "Failed to make blank_screen_now.sh executable"
    fi
...
setup_screen_blanking() {
    log_info "Setting up screen blanking (ultimate fix)..."
    
    local blank_script="${INSTALL_DIR}/fix_blank_screen.sh"
    local blank_now_script="${INSTALL_DIR}/blank_screen_now.sh"
    local blank_service="${INSTALL_DIR}/blank-screen.service"
    
    # Check if files exist
    if [[ -f "${blank_script}" ]] && [[ -f "${blank_service}" ]]; then
        # Copy scripts to system location
        sudo cp "${blank_script}" /usr/local/bin/ || error_exit "Failed to copy blank screen script"
        sudo chmod +x /usr/local/bin/fix_blank_screen.sh || error_exit "Failed to make blank screen script executable"
        
        # Copy manual blank script
        if [[ -f "${blank_now_script}" ]]; then
            sudo cp "${blank_now_script}" /usr/local/bin/ || error_exit "Failed to copy manual blank script"
            sudo chmod +x /usr/local/bin/blank_screen_now.sh || error_exit "Failed to make manual blank script executable"
        fi
        
        # Copy service file
        sudo cp "${blank_service}" /etc/systemd/system/ || error_exit "Failed to copy blank screen service"
        
        # Reload systemd and enable service
        sudo systemctl daemon-reload || error_exit "Failed to reload systemd"
        sudo systemctl enable blank-screen.service || log_warning "Failed to enable blank screen service"
        
        # Apply the ultimate fix directly during installation
        log_info "Applying ultimate screen blanking fix..."
        ...
        if [[ -n "${cmdline_file}" ]]; then
            # Backup
            sudo cp "${cmdline_file}" "${cmdline_file}.backup.$(date +%s)" || true
            ...
            echo "$modified" | sudo tee "${cmdline_file}" > /dev/null
            
            log_success "Kernel parameters updated for silent boot"
        fi
        ...
        # Configure systemd for no console output
        sudo mkdir -p /etc/systemd/system.conf.d/
        sudo tee /etc/systemd/system.conf.d/no-console-output.conf > /dev/null <<'EOF'
[Manager]
ShowStatus=no
LogLevel=err
LogTarget=journal
...
EOF
        ...
        if [[ -n "${config_file}" ]]; then
            sudo cp "${config_file}" "${config_file}.backup.$(date +%s)" 2>/dev/null || true
        ...
        # Start service (will blank screen immediately)
        sudo systemctl start blank-screen.service || log_warning "Failed to start blank screen service"
        
        # Run the script immediately to blank screen now
        sudo /usr/local/bin/fix_blank_screen.sh || log_warning "Failed to run blank screen script"
        
        log_success "Screen blanking configured (ultimate fix applied)"
        log_info "Note: Reboot required for silent boot to take full effect"
    else
        log_warning "Screen blanking scripts not found, skipping (display may show console text)"
        log_info "This is optional - the player will work without screen blanking"
    fi
}
...
main() {
    ...
    echo "   Silent boot (blank screen) will only work after reboot"
    ...
    echo "  If you see console text, run: sudo /usr/local/bin/blank_screen_now.sh"
    echo ""
