...
class MPVController:
    ...
    def _get_mpv_command(self, file_path: str) -> List[str]:
        ...
        cmd = [
            "mpv",
            "--no-terminal",
            "--msg-level=all=info",  # Show info messages for debugging
        ...
        # Use SDL instead of GPU/DRM for better compatibility with 4K TVs
        cmd.extend([
            "--vo=sdl",
        ])
        
        # Audio configuration - Force HDMI audio output
        # When using SDL video output, audio needs to be explicitly configured
        cmd.extend([
            "--audio-device=alsa/hdmi:CARD=vc4hdmi0,DEV=0",  # HDMI-1 audio
        ...
        cmd.extend([
            ...
            "--demuxer-max-bytes=50M",  # Limit cache size
        ])
